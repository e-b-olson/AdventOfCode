...
def parse_input():
    ...
    with open("puzzle_19_test_01_input.txt", "r") as f:
        data = f.readlines()
    f.close()
...
def part_02_process_bp(bp, t=24):
    ...
    def process_bp_recurse(bp, resources, robots, time):
        ...
        # update resources
        production = (robots[ORE], robots[CLA], robots[OBS])
        ...
        # skip building a robot (but limit this with best guess)
        if resources[0] < 6 or resources[2] < max_obs:
            option = process_bp_recurse(bp, tuple(next_resources), robots, time - 1)
            options.append(option)

        sorted_options = sorted(options)
        return robots[GEO] + sorted_options[-1]

    return process_bp_recurse(tuple(bp), (0, 0, 0), (1, 0, 0, 0), t)
